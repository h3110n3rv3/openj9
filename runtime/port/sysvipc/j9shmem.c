...
j9shmem_getDir(struct J9PortLibrary* portLibrary, const char* ctrlDirName, uint32_t flags, char* buffer, uintptr_t bufLength)
{
	...
	/*
	 ...
	 return the user's home directory - $HOME/ - only.
	*/
	if (J9_ARE_ALL_BITS_SET(flags, J9SHMEM_GETDIR_DO_NOT_APPEND_HIDDENDIR)) {
		...
		if (homeDir != NULL) {
			if (strlen(homeDir) + 1 > bufLength) {
				rc = J9PORT_ERROR_SHMEM_GET_DIR_BUF_OVERFLOW;
				goto exit;
			}
			if (homeDir[strlen(homeDir) - 1] != '/') {
				omrstr_printf(buffer, bufLength, "%s/", homeDir);
			} else {
				omrstr_printf(buffer, bufLength, "%s", homeDir);
			}
			goto exit;
		...
		if (J9_ARE_ALL_BITS_SET(flags, J9SHMEM_GETDIR_USE_USERHOME)) {
			...
			if (NULL == homeDir) {
				...
				if (NULL != pwent) {
					...
					if ((0 < dirLen) && ((dirLen + baseDirLen) < minBufLen)) {
						homeDir = pwent->pw_dir;
					} else {
