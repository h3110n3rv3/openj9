...
createDirectory(struct J9PortLibrary *portLibrary, char *pathname, uintptr_t permission)
{
	...
	if (0 == j9shmem_getDir(portLibrary, NULL, J9SHMEM_GETDIR_DO_NOT_APPEND_HIDDENDIR, homeDir, J9SH_MAXPATH)) {
		if ((pathname != NULL) && (strncmp(pathname, homeDir, strlen(homeDir)) == 0)) {
			usingUserHome = TRUE;
		}
	}
	/* If we are using user home and the permission is "absent", override permission.
		Otherwise, leave permission unchanged. */
	if ((J9SH_DIRPERM_ABSENT == permission) || (J9SH_DIRPERM_ABSENT_GROUPACCESS == permission)) {
		if (usingUserHome) {
			permission = J9SH_DIRPERM_CACHE;
		} else {
	...
	while ((NULL != current) && (omrfile_attr(pathname) != EsIsDir)) {
		...
		*current = '\0';
		...
		if (0 == omrfile_mkdir(tempPath)) {
			...
			/* check whether the directory has already been created */
