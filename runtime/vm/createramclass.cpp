...
// Use the fast/slow pointer method to split the list.
...
J9RAMClassFreeListBlock *mid = slow->nextFreeListBlock;
...
while (current != NULL) {
	while (current->nextFreeListBlock &&
...
if (NULL != requests) {
	...
	} else if(FREQUENTLY_ACCESSED == segmentKind)
	{
		memoryType = MEMORY_TYPE_RAM_CLASS_FREQUENT;
	} else if(INFREQUENTLY_ACCESSED == segmentKind)
	{
		memoryType = MEMORY_TYPE_RAM_CLASS_INFREQUENT;
	}
