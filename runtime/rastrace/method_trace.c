	...
	if (isCompiled) {
		...
		if (modifiers & J9AccStatic) {
			...
			if (modifiers & J9AccNative) {
				...
				Trc_MethodEntryStr(thr, J9UTF8_LENGTH(className), J9UTF8_DATA(className), J9UTF8_LENGTH(methodName), J9UTF8_DATA(methodName), J9UTF8_LENGTH(methodSignature), J9UTF8_DATA(methodSignature), receiver, 0, "");
			}
			...
			if (doParameters) {
				...
				if ((receiverClazz == J9VMJAVALANGSTRING_OR_NULL(vm))
					...
					Trc_MethodEntryStr(thr, J9UTF8_LENGTH(className), J9UTF8_DATA(className), J9UTF8_LENGTH(methodName), J9UTF8_DATA(methodName), J9UTF8_LENGTH(methodSignature), J9UTF8_DATA(methodSignature), receiver, J9UTF8_LENGTH(outputString), outputString);
